...
> Go to `settings.py` and add it on `INSTALLED_APPS`.
 ...
 Type `python manage.py migrate` on terminal to apply pending migrations to your database schema. it takes the Python instructions generated by makemigrations and executes the corresponding SQL commands to build or modify your database tables.